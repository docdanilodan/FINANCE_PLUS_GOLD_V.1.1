...
def _write_audit_record(
    ...
    _response_json(session.put(url, json=payload, timeout=30))
...
def main() -> int:
    ...
    jobs_url = f"{api_url}/repos/{repository}/actions/runs/{run['id']}/jobs"
    jobs = _response_json(session.get(jobs_url, params={"filter": "latest", "per_page": 100}, timeout=30))
