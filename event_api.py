...
def _external(
    ...
    # A runtime token is accepted only by endpoints that have already passed
    # GitHub OIDC/secret authentication. It is never persisted or logged.
    airtable = AirtableGold(token=airtable_token) if airtable_token else AirtableGold()
...
def receive_gmail_event(
    payload: ExternalEventPayload,
    x_financeplus_webhook_secret: str | None = Header(default=None),
    x_financeplus_airtable_token: str | None = Header(default=None),
    authorization: str | None = Header(default=None),
) -> dict:
    _authorize_external(x_financeplus_webhook_secret, authorization)
    return _external("Gmail", payload, x_financeplus_airtable_token)
...
def receive_github_event(
    ...
    _authorize_external(x_financeplus_webhook_secret, authorization)
    return _external("GitHub", payload, x_financeplus_airtable_token)
