...
def match_client_practice(airtable: AirtableGold, text: str) -> MatchResult:
    hay = _norm(text)
    if not hay:
        return MatchResult()
    clients = airtable.list_records("clienti", max_records=2000)
    scored = []
    for rec in clients:
        f = rec.get("fields", {})
        candidates = {
        ...
        for field, value in candidates.items():
            ...
            if token and len(token) >= 5 and token in hay:
                candidate_score = 0.99 if field in ("Partita IVA", "Codice Fiscale") else 0.92 if field in ("PEC", "REA") else 0.80
    ...
    result = MatchResult(client_id=client["id"], client_name=cf.get("Cliente"), confidence=score, reason=reason)
    practices = airtable.list_records("pratiche", max_records=2000)
    practice_candidates = []
    for rec in practices:
        f = rec.get("fields", {})
        linked = f.get("Cliente collegato", [])
        if result.client_id in linked or _norm(str(f.get("Cliente", ""))) == _norm(str(result.client_name or "")):
    ...
    if practice_candidates:
        ...
        result.confidence = min(result.confidence, pscore) if pscore >= 0.80 else result.confidence
        result.reason += "; pratica associata"
    return result


def missing_documents_for_practice(airtable: AirtableGold, practice_record: dict) -> list[str]:
    ...
    present = {str(d.get("fields", {}).get("Tipo Documento", "")).casefold() for d in docs}
